...
const RACE_WALLET_ADDRESS = 'ESwYNvNieSniHZKAw2MezWi7niYmtZbjtKUR8KyaySiv';
const MIN_BET = 0.01;
const MAX_BET = 1.0;
const LOG_DIR = './data';
...
bot.onText(/^\/confirm$/, async (msg) => {
    ...
    try {
        ...
        if (!paymentCheckResult.success) {
            ...
            return await bot.sendMessage(chatId, `⚠️ Payment not verified! ${paymentCheckResult.message}`);
        }
        ...
        if (win) {
            ...
            const sendResult = await sendSol(connection, process.env.BOT_PRIVATE_KEY, winnerPublicKey, payout);
        ...
        if (error.message && error.message.includes('429')) {
            await bot.sendMessage(chatId, `⚠️ We're hitting rate limits from the Solana network. Please try again shortly.`);
        } else {
            await bot.sendMessage(chatId, `⚠️ An internal error occurred during confirmation.`);
        }
    }
});
...
bot.onText(/^\/confirmrace$/, async (msg) => {
    ...
    try {
        ...
        const paymentCheckResult = await checkPayment(amount, userId, 'race', RACE_WALLET_ADDRESS);

        if (!paymentCheckResult.success) {
            return bot.sendMessage(chatId, `⚠️ Payment not verified for Race ${raceId}! ${paymentCheckResult.message}`);
        }

        if (!userPayments[userId]) {
            userPayments[userId] = {};
        }
        userPayments[userId].raceTx = paymentCheckResult.tx;
        ...
        if (horse === winningHorse.name) {
            ...
            try {
                ...
                if (!racePayerPrivateKey) {
                    ...
                    return await bot.sendMessage(chatId, `⚠️ Race payout failed: Bot's race private key not configured.`);
                }
                ...
                if (paymentCheckResult && paymentCheckResult.tx) {
                    try {
                        ...
                        if (parsedTransaction && parsedTransaction.transaction && parsedTransaction.transaction.message && parsedTransaction.transaction.message.accountKeys && parsedTransaction.transaction.message.length > 0) {
                            ...
                            if (!winnerPublicKey) {
                                console.warn('Could not determine the sender from the transaction.');
                                return await bot.sendMessage(chatId, `⚠️ Race payout failed: Could not determine payment sender.`);
                            }
                            const winnerAddress = winnerPublicKey.toBase58();
                            if (linkedWallets[userId] && linkedWallets[userId] !== winnerAddress) {
                                return await bot.sendMessage(chatId, `⚠️ This wallet does not match your linked wallet. Please use your original address.`);
                            ...
                            return await bot.sendMessage(chatId, `⚠️ Race payout failed: Could not analyze your payment transaction.`);
                        }
                    } catch (error) {
                        console.error('Error parsing transaction for sender:', error);
                        return await bot.sendMessage(chatId, `⚠️ Race payout failed: Error analyzing your payment transaction.`);
                    }
                } else {
                    console.warn('No transaction signature available to determine sender.');
                    return await bot.sendMessage(chatId, `⚠️ Race payout failed: No payment transaction found.`);
                }

                if (!winnerPublicKey) {
                    console.warn('Winner public key is undefined.');
                    return await bot.sendMessage(chatId, `⚠️ Race payout failed: Could not determine recipient.`);
                }

                const sendResult = await sendSol(connection, racePayerPrivateKey, winnerPublicKey, payout);

                if (sendResult.success) {
                    await bot.sendMessage(chatId, `💰 Race winnings of ${payout.toFixed(4)} SOL sent! TX: ${sendResult.signature}`);
                } else {
                    await bot.sendMessage(chatId, `⚠️ Race payout failed: ${sendResult.error}`);
                }

            } catch (error) {
                console.error('Error during race payout:', error);
                await bot.sendMessage(chatId, `⚠️ Race payout failed due to an error.`);
            }
        ...
        if (userPayments[userId] && userPayments[userId].raceTx === paymentCheckResult.tx) {
            delete userPayments[userId].raceTx;
        }
