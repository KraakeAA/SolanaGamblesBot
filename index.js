...
const OPTIONAL_ENV_DEFAULTS = { /* ... all the defaults as listed before ... */
    ...
    'SLOTS_JACKPOT_SEED_LAMPORTS': '100000000', 'SLOTS_JACKPOT_CONTRIBUTION_PERCENT': '0.001', 'REFERRAL_INITIAL_BET_MIN_LAMPORTS': '100000000',
...
// In Part 5a (Telegram Message/Callback Handlers & Game Result Processing)
...
        if (callbackQuery && callbackQuery.id) { await bot.answerCallbackQuery(callbackQuery.id, { text: "Error processing action.", show_alert: true }).catch(()=>{}); }
        return;
    }
    const userId = String(callbackQuery.from.id); 
    const chatId = String(callbackQuery.message.chat.id);
    const messageId = callbackQuery.message.message_id; // ID of the message where the button was clicked
...
        await safeSendMessage(chatId, "A database error occurred\\. Please try again later\\.", { parse_mode: 'MarkdownV2' }); 
        if (tempClient) tempClient.release();
        return;
    } finally {
        if (tempClient) tempClient.release();
...
                await safeSendMessage(chatId, `Unknown menu option: \`${escapeMarkdownV2(menuKey)}\``, { parse_mode: 'MarkdownV2'});
            }
        }
...
                await bot.editMessageText(`⚠️ Unknown game: \`${escapeMarkdownV2(gameKey)}\``, { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2' });
            }
        }
        else if (action === 'roulette_select_bet_type' || action === 'roulette_bet_type_category') {
            const gameKey = 'roulette';
            console.log(`${logPrefix} Routing intermediate step for Roulette.`);
...
                await bot.editMessageText("⚠️ Error processing game step selection\\.", { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2' });
            }
        }
...
                await bot.editMessageText("⚠️ Error confirming bet due to invalid parameters\\.", { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2' });
                return;
            }
...
            const gameKey = params[0];
            if (GAME_CONFIG[gameKey]) { await handleCustomAmountSelection(userId, chatId, messageId, gameKey); } 
            else { console.warn(`${logPrefix} Unknown game key for custom amount: ${gameKey}`); await bot.editMessageText(`⚠️ Unknown game for custom amount: \`${escapeMarkdownV2(gameKey)}\``, { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2' }); }
        }
...
                    await bot.editMessageText("⚠️ Blackjack game session expired or invalid\\. Please start a new game\\.", { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [[{ text: '↩️ Back to Games', callback_data: 'menu:game_selection' }]] } });
                }
...
                    await bot.editMessageText(cashoutSuccessMsg, { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [[{ text: '🔄 Play Again', callback_data: `play_again:crash:${betAmountN}` }, { text: '🎮 Games Menu', callback_data: 'menu:game_selection' }]] } }).catch(e => {});
                } catch (dbError) {
...
                await bot.editMessageText("⚠️ Too late to cash out or game already ended\\.", { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [[{ text: `🔄 Play Crash Again`, callback_data: `play_again:crash:${gameState?.betAmountLamports || GAME_CONFIG.crash.minBetLamports}` }, { text: '🎮 Games Menu', callback_data: 'menu:game_selection' }]] } });
            }
        }
...
                console.warn(`${logPrefix} No valid 'awaiting_withdrawal_confirmation' state found for user ${userId} or state data missing. Prior state:`, priorState);
                await bot.editMessageText("⚠️ Your withdrawal session seems to have expired or is invalid\\. Please start over by using the /withdraw command or Wallet menu\\.", { 
                    chat_id: chatId, 
                    message_id: messageId, 
                    parse_mode: 'MarkdownV2', 
...
            const { linkedWallet, amountLamportsStr } = priorState.data; 
            clearUserState(userId); 
...
// --- End Main Callback Query Handler ---// --- End Main Callback Query Handler ---
...
            amountToCreditToUser = betAmountLamports + profitOnWin; 
...
        await sleep(2000); 
        ...
        const OTHER_FINISHER_CHAR = '🥈';   // MODIFIED: For other finishers (Bronze was 🥉, Silver is 🥈)
        ...
        const animationDurationApproxMs = 7000; // Adjusted for shorter track
        const framesPerSecond = 3; // Slightly faster updates for smoother feel on shorter track
        ...
        for (let frame = 0; frame < totalAnimationFrames; frame++) {
            ...
            for (let i = 0; i < RACE_HORSES.length; i++) {
                if (positions[i] < VIRTUAL_TRACK_LENGTH) {
                    ...
                    let currentHorseBaseProb = 0.4 + (1.0 - (RACE_HORSES[i].payoutMultiplier / 10)); // Adjusted for VTL=30
                    currentHorseBaseProb = Math.max(0.15, Math.min(1.0, currentHorseBaseProb));


                    if (randomFactor < currentHorseBaseProb * 0.25) moveAmount = 3; // Higher chance of bigger moves     
                    else if (randomFactor < currentHorseBaseProb * 0.60) moveAmount = 2;  
                    else if (randomFactor < currentHorseBaseProb) moveAmount = 1;        
                    
                    if ((i + 1) === winningLane) { 
                        moveAmount = Math.max(moveAmount, 1); 
                        if (frame > totalAnimationFrames * 0.3) { 
                            moveAmount = Math.max(moveAmount, (randomFactor < 0.65 ? 2 : 1) + (randomFactor < 0.30 ? 1: 0) ); 
                        }
                        if (frame >= totalAnimationFrames - Math.max(3, Math.ceil(VISUAL_TRACK_SLOTS/2)) && positions[i] < VIRTUAL_TRACK_LENGTH) { // Ensure winner finishes
                           let needed = VIRTUAL_TRACK_LENGTH - positions[i];
                           let framesLeft = Math.max(1, totalAnimationFrames - frame);
                           moveAmount = Math.max(moveAmount, Math.ceil(needed / framesLeft), 1); // Ensure at least 1 to finish
                           console.log(`${logPrefix} Winner ${RACE_HORSES[i].name} final push. Pos: ${positions[i]}, Needs: ${needed}, Move: ${moveAmount}`);
                        }
                    } else { 
                        if (positions[winningLane-1] > VIRTUAL_TRACK_LENGTH * 0.6 && positions[i] < positions[winningLane-1] * 0.5 && Math.random() < 0.4) {
                            moveAmount = Math.min(moveAmount, 1); 
                        }
                        if (frame > totalAnimationFrames * 0.8 && positions[winningLane-1] >= VIRTUAL_TRACK_LENGTH - (VIRTUAL_TRACK_LENGTH/VISUAL_TRACK_SLOTS)) {
                            if (moveAmount > 1 && Math.random() < 0.6) moveAmount = 1; 
                        }
                    }
                ...
                } else if (positions[i] < VIRTUAL_TRACK_LENGTH && visualMarkerSlot === VISUAL_TRACK_SLOTS - 1) {
                    // If horse is on the last slot but not virtually finished, HORSE_MARKER_CHAR is already placed there by the logic above.
                    // No change needed here, the FINISH_LINE_CHAR is effectively "covered" by the HORSE_MARKER_CHAR.
                }
                // If finished, the trophy/finisher char is already placed in the last slot.
            ...
            if (winnerPhysicallyFinished && frame >= Math.floor(totalAnimationFrames * 0.70) ) { // Check if winner finished and decent number of frames passed
                 console.log(`${logPrefix} Winner finished and sufficient frames passed. Ending animation.`);
                 break; 
            }
            if (!someHorseStillRacing && frame > totalAnimationFrames * 0.3) { // If all horses somehow finished very early
                console.log(`${logPrefix} All horses visually finished early. Animation ending.`);
                break;
            }
...
async function handleWarGame(userId, chatId, messageId, betAmountLamports) {
    ...
    console.log(`${logPrefix} Handling war game with corrected visuals.`);

    let client = null;
    let finalUserBalance;
    let playerCard, dealerCard, playerRank, dealerRank, gameResultOutcome;

    const cardSpinChars = ['❔', '🎴', '❔'];
    const spinIterations = 3;
    const spinSleepMs = 220; // Slightly adjusted
    const revealPauseMs = 1300; // Slightly adjusted
    ...
    try {
        ...
        gameResultOutcome = simulation.result;
        ...
        if (messageId) {
            ...
            if (tempMsg) messageId = tempMsg.message_id;
        ...
        let playerDisplayValue = ""; // This will hold the card string like "K♠️" or spin char
        for (let i = 0; i < spinIterations; i++) {
            playerDisplayValue = escapeMarkdownV2(cardSpinChars[i % cardSpinChars.length]);
            currentMessageText = `🃏 *Casino War* 🃏\n\nBet: ${betAmountSOLFormatted} SOL\n\n` +
                                 `Player: \\[ ${playerDisplayValue} \\]\n` + // Literal brackets escaped
                                 `Dealer: \\[   \\]\n\nDealing your card\\.\\.\\.`; // Escaped ...
        ...
        playerDisplayValue = `*${escapeMarkdownV2(playerCard)}*`; // Actual card, bolded and escaped
        const playerRankFormatted = `*${escapeMarkdownV2(String(playerRank))}*`; // Bold rank
        currentMessageText = `🃏 *Casino War* 🃏\n\nBet: ${betAmountSOLFormatted} SOL\n\n` +
                             `Player: \\[ ${playerDisplayValue} \\] \\(${playerRankFormatted}\\)\n` + // Escaped ( )
                             `Dealer: \\[   \\]\n\nDealing dealer's card\\.\\.\\.`; // Escaped ...
        ...
        for (let i = 0; i < spinIterations; i++) {
            dealerDisplayValue = escapeMarkdownV2(cardSpinChars[i % cardSpinChars.length]);
            currentMessageText = `🃏 *Casino War* 🃏\n\nBet: ${betAmountSOLFormatted} SOL\n\n` +
                                 `Player: \\[ ${playerDisplayValue} \\] \\(${playerRankFormatted}\\)\n` +
                                 `Dealer: \\[ ${dealerDisplayValue} \\]\n\nRevealing dealer's card\\.\\.\\.`; // Escaped ...
        ...
        dealerDisplayValue = `*${escapeMarkdownV2(dealerCard)}*`; // Actual card, bolded and escaped
        const dealerRankFormatted = `*${escapeMarkdownV2(String(dealerRank))}*`; // Bold rank
        currentMessageText = `🃏 *Casino War* 🃏\n\nBet: ${betAmountSOLFormatted} SOL\n\n` +
                             `Player: \\[ ${playerDisplayValue} \\] \\(${playerRankFormatted}\\)\n` +
                             `Dealer: \\[ ${dealerDisplayValue} \\] \\(${dealerRankFormatted}\\)\n\nDetermining winner\\.\\.\\.`; // Escaped ...
        ...
        if (gameResultOutcome === 'win') {
            ...
            payoutAmountForDB = betAmountLamports;
        ...
        const finalMessageText = `🃏 *Casino War Result* 🃏\n\nBet: ${betAmountSOLFormatted} SOL\n\n` +
                                 `Player: \\[ ${playerDisplayValue} \\] \\(${playerRankFormatted}\\)\n` + // playerDisplayValue already bolded & escaped
                                 `Dealer: \\[ ${dealerDisplayValue} \\] \\(${dealerRankFormatted}\\)\n\n` + // dealerDisplayValue already bolded & escaped
...
async function handleCrashGame(userId, chatId, messageId, betAmountLamports) {
    ...
    console.log(`${logPrefix} Starting Crash game with MORE engaging visuals.`);

    let client = null;
    let betId = null;
    let balanceAfterBet = null;
    let lastMessageText = "";
    let currentMessageIdForAnimation = messageId;
    ...
    try {
        ...
        if (!betPlacementResult.success) {
            ...
            if (currentMessageIdForAnimation) {
                await bot.editMessageText(errorMsg, { chat_id: chatId, message_id: currentMessageIdForAnimation, parse_mode: 'MarkdownV2', reply_markup: errorKeyboardBet})
                    .catch(async e => await safeSendMessage(chatId, errorMsg, {parse_mode: 'MarkdownV2', reply_markup: errorKeyboardBet}));
            } else {
                await safeSendMessage(chatId, errorMsg, {parse_mode: 'MarkdownV2', reply_markup: errorKeyboardBet});
            }
        ...
        const initialBetAmountFormatted = escapeMarkdownV2(formatSol(betAmountLamports));
        const initialMultiplierFormatted = escapeMarkdownV2("1.00");
        const initialAnimationText = `🚀 *Crash Game Starting\\!* 🚀\n\nBet: ${initialBetAmountFormatted} SOL\nMultiplier: *${initialMultiplierFormatted}x*\n\nIgnition sequence confirmed\\! Countdown\\.\\.\\. 3\\.\\.\\. 2\\.\\.\\. 1\\.\\.\\.`; // Escaped ! ...
        try {
            if (currentMessageIdForAnimation) {
                ...
                else throw new Error("Failed to send initial Crash message for animation.");
            }
        } catch (e) {
            ...
            if (!currentMessageIdForAnimation) {
                ...
                throw new Error("Failed to establish message for Crash animation\\.");
            }
        }
        ...
        userStateCache.set(userId, {
            ...
            messageId: currentMessageIdForAnimation, currentMultiplier: 1.00,
        ...
        while (loopActive && currentMessageIdForAnimation) {
            const gameState = userStateCache.get(userId);
            ...
            let increment = 0.01;
            if (oldMultiplier >= 50) increment = 0.75 + (Math.random() * 0.5);
            else if (oldMultiplier >= 25) increment = 0.50 + (Math.random() * 0.25);
            else if (oldMultiplier >= 10) increment = 0.20 + (Math.random() * 0.1);
            else if (oldMultiplier >= 5) increment = 0.08 + (Math.random() * 0.04);
            else if (oldMultiplier >= 2) increment = 0.03 + (Math.random() * 0.02);
            ...
            const multiplierForButtonText = currentMultiplier.toFixed(2);
            const multiplierForMessageBody = escapeMarkdownV2(currentMultiplier.toFixed(2));
            ...
            if (currentMultiplier < 1.2) {
                emojiTrail = `🚀`;
                statusText = `Ignition sequence confirmed\\! Countdown\\.\\.\\. 3\\.\\.\\. 2\\.\\.\\. 1\\.\\.\\.`;
            } else if (currentMultiplier < 1.5) {
                emojiTrail = `💨🚀`;
                statusText = `We have liftoff\\! Main thrusters engaged\\! 🌍`;
            } else if (currentMultiplier < 2.0) {
                emojiTrail = `💨💨🚀`;
                statusText = `Climbing smoothly\\! Altitude: increasing\\! 📈`;
            } else if (currentMultiplier < 3.0) {
                emojiTrail = `✨💨💨🚀`;
                statusText = `Breaking through low atmosphere\\! Looking good\\! 👍`;
            } else if (currentMultiplier < 4.0) {
                emojiTrail = `✨✨💨🚀`;
                statusText = `Approaching the stratosphere\\! View is amazing\\! 🤩`;
            } else if (currentMultiplier < 6.0) {
                emojiTrail = `💥✨✨🚀`;
                statusText = `Engines at full power\\! Pushing the limits\\! 🔥`;
            } else if (currentMultiplier < 10.0) {
                emojiTrail = `💥💥✨🚀`;
                statusText = `Entering mesosphere\\! Getting risky now\\! 😬`;
            } else if (currentMultiplier < 15.0) {
                emojiTrail = `🔥🔥💥🚀`;
                statusText = `High orbit achieved\\! Are you brave enough to continue\\?! 🌌`;
            } else if (currentMultiplier < 25.0) {
                emojiTrail = `🔥🔥🔥🚀`;
                statusText = `Deep space voyage\\! Uncharted territory\\! 🌠 This is extreme\\!`;
            } else if (currentMultiplier < 50.0) {
                emojiTrail = `🌌🔥🔥🔥🚀`;
                statusText = `Ludicrous Speed\\! Systems shaking\\! 💠 Danger Zone\\!`;
            } else {
                emojiTrail = `🚨🌌🔥🔥🚀🚨`;
                statusText = `WARNING\\! CRITICAL VELOCITY\\! Unstable\\! Cash out NOW or face the void\\! 😱`;
            }

            let animationText = `${emojiTrail} *Crash Game In Progress\\!* ${emojiTrail}\n\n` +
                                `Bet: ${initialBetAmountFormatted} SOL\n` +
                                `Multiplier: *${multiplierForMessageBody}x*\n\n` +
                                `${statusText}`; // statusText is already pre-escaped if it were dynamic with user content. Here it's static but with special chars.
            ...
            const delay = Math.max(80, 450 - Math.floor(oldMultiplier * 15)); // Adjusted delay curve for more updates
            await sleep(delay);
        } // End while loop
        ...
        if (finalStateCheck && finalStateCheck.action === 'awaiting_crash_cashout' && finalStateCheck.betId === betId) {
            ...
            try {
                ...
                const escapedActualCrashPoint = escapeMarkdownV2(actualCrashPoint.toFixed(2));
                const crashResultText = `💥 *CRASHED at ${escapedActualCrashPoint}x\\!* 💥\n\n` +
                                        `Bet: ${initialBetAmountFormatted} SOL\n` +
                ...
                const crashKeyboard = { inline_keyboard: [[{ text: '🔄 Play Again', callback_data: `play_again:${gameKey}:${betAmountLamports}` }, { text: '🎮 Games Menu', callback_data: 'menu:game_selection' }]] };

                if (currentMessageIdForAnimation) {
                    await bot.editMessageText(crashResultText, { chat_id: chatId, message_id: currentMessageIdForAnimation, parse_mode: 'MarkdownV2', reply_markup: crashKeyboard })
                        .catch(async e => {
                            console.warn(`${logPrefix} Failed to edit crash result message, sending new: ${e.message}`);
                            await safeSendMessage(chatId, crashResultText, { parse_mode: 'MarkdownV2', reply_markup: crashKeyboard });
                        });
                } else {
                     await safeSendMessage(chatId, crashResultText, { parse_mode: 'MarkdownV2', reply_markup: crashKeyboard });
                }
                ...
                const dbErrorText = `⚠️ Database error after crash for Bet ID ${betId}: ${escapeMarkdownV2(dbError.message)}\\. Your bet was lost\\. Please contact support if balance appears incorrect\\.`;
                 if (currentMessageIdForAnimation) {
                    bot.editMessageText(dbErrorText, { chat_id: chatId, message_id: currentMessageIdForAnimation, parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [[{ text: `🔄 Play Crash Again`, callback_data: `play_again:${gameKey}:${betAmountLamports}` }, { text: '🎮 Games Menu', callback_data: 'menu:game_selection' }]] } })
                        .catch(async e => await safeSendMessage(chatId, dbErrorText, {parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [[{ text: `🔄 Play Crash Again`, callback_data: `play_again:${gameKey}:${betAmountLamports}` }, { text: '🎮 Games Menu', callback_data: 'menu:game_selection' }]] }}));
                } else {
            ...
            console.log(`${logPrefix} Crash game loop ended. User state already cleared. Bet ID: ${betId || 'N/A'}.`);
        }
        ...
        const idToEdit = currentMessageIdForAnimation || messageId;
        if (idToEdit) {
            await bot.editMessageText(errorTextCrash, { chat_id: chatId, message_id: idToEdit, parse_mode: 'MarkdownV2', reply_markup: errorKeyboardCrash })
                .catch(async e => await safeSendMessage(chatId, errorTextCrash, {parse_mode: 'MarkdownV2', reply_markup: errorKeyboardCrash }));
        } else {
...
        clearUserState(userId);
        await safeSendMessage(chatId, "An internal error occurred processing your amount input \\(context missing\\)\\. Please start withdrawal again\\.", { parse_mode: 'MarkdownV2'});
        return;
    }
...
    if (originalMessageId && originalMessageId !== msg.message_id) {
        await bot.deleteMessage(chatId, originalMessageId).catch(e => console.warn(`${logPrefix} Non-critical: Failed to delete prompt message ${originalMessageId}: ${e.message}`));
    }
    await bot.deleteMessage(chatId, msg.message_id).catch(e => console.warn(`${logPrefix} Non-critical: Failed to delete user input message ${msg.message_id}: ${e.message}`));
...
            await safeSendMessage(chatId, minErrorText, {
                parse_mode: 'MarkdownV2',
                reply_markup: { inline_keyboard: [[{ text: '↩️ Back to Wallet', callback_data: 'menu:wallet' }]] }
            });
            return;
...
            const insufficientText = `${escapeMarkdownV2(breadcrumb)}\nInsufficient balance for withdrawal of ${escapeMarkdownV2(formatSol(amountLamports))} SOL \\(Total needed including fee: ${escapeMarkdownV2(formatSol(totalDeductionLamports))} SOL\\)\\. Your balance: ${escapeMarkdownV2(formatSol(currentBalance))} SOL\\. The fee is ${escapeMarkdownV2(formatSol(feeLamports))} SOL\\. Or /cancel\\.`;
            await safeSendMessage(chatId, insufficientText, {
                parse_mode: 'MarkdownV2',
                reply_markup: { inline_keyboard: [[{ text: '↩️ Back to Wallet', callback_data: 'menu:wallet' }]] }
            });
            return;
...
        const finalConfirmationBreadcrumb = `${escapeMarkdownV2(breadcrumb)} \\> Confirm ${amountSOLFormatted} SOL`; // Escaped >

        const confirmationText = `*Confirm Withdrawal*\n\n` +
                                 `${finalConfirmationBreadcrumb}\n` + 
...
        ];
        ...
        const sentConfirmationMsg = await safeSendMessage(chatId, confirmationText, { 
            parse_mode: 'MarkdownV2', 
            reply_markup: { inline_keyboard: inlineKeyboard } 
        });

        if (sentConfirmationMsg && sentConfirmationMsg.message_id) {
            userStateCache.set(userId, {
                state: 'awaiting_withdrawal_confirmation', 
                chatId: String(chatId),
                messageId: sentConfirmationMsg.message_id, 
                data: {
            ...
            console.log(`${logPrefix} Sent withdrawal confirmation. User state set to 'awaiting_withdrawal_confirmation'.`);
        } else {
            console.error(`${logPrefix} Failed to send withdrawal confirmation message. Cannot set 'awaiting_withdrawal_confirmation' state.`);
            await safeSendMessage(chatId, "⚠️ Error displaying withdrawal confirmation options. Please try starting the withdrawal again.", {parse_mode: 'MarkdownV2'});
        }

    } catch (e) {
        console.error(`${logPrefix} Error processing withdrawal amount: ${e.message}`, e); 
        const errorText = `${escapeMarkdownV2(breadcrumb)}\nInvalid amount: "${escapeMarkdownV2(textAmount)}"\\. ${escapeMarkdownV2(e.message)}\\.\nPlease enter a valid withdrawal amount or /cancel\\.`;
...
/**
 * Handles the core withdrawal process AFTER user has clicked "Yes" on the confirmation.
 * This function is called by handleCallbackQuery from the 'process_withdrawal_confirm:yes' action.
 * @param {string} userId The user's Telegram ID.
 * @param {string} chatId The chat ID.
 * @param {number} confirmationMessageId The ID of the message that had the Yes/No buttons (to be edited).
 * @param {string} recipientAddress The Solana address to send to.
...
async function handleWithdrawalConfirmation(userId, chatId, confirmationMessageId, recipientAddress, amountLamportsStr) {
    const logPrefix = `[HandleWithdrawConfirm User ${userId}]`; // Renamed logPrefix slightly
    console.log(`${logPrefix} Confirmed. Addr: ${recipientAddress}, AmtStr: ${amountLamportsStr}, ConfirmMsgID: ${confirmationMessageId}`);

    const backToWalletKeyboard = { inline_keyboard: [[{text: "↩️ Back to Wallet", callback_data: "menu:wallet"}]] };
    ...
    try {
        ...
        if (currentBalance < totalDeductionLamports) {
            const insufficientText = `⚠️ Insufficient balance for withdrawal at the time of processing\\. Needed: ${escapeMarkdownV2(formatSol(totalDeductionLamports))} SOL, Have: ${escapeMarkdownV2(formatSol(currentBalance))} SOL\\. Withdrawal cancelled\\.`;
            if (confirmationMessageId) {
                await bot.editMessageText(insufficientText, { chat_id: chatId, message_id: confirmationMessageId, parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard });
            } else {
                await safeSendMessage(chatId, insufficientText, { parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard });
            }
            return;
        }

        if (confirmationMessageId) {
            await bot.editMessageText("⏳ Processing your withdrawal request\\.\\.\\. This may take a moment\\.", { 
                chat_id: chatId, message_id: confirmationMessageId, parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [] }
            }).catch(e => console.warn(`${logPrefix} Non-critical: Error editing message ${confirmationMessageId} to processing state: ${e.message}`));
        } else {
            console.warn(`${logPrefix} No confirmationMessageId provided to edit to 'Processing...'. Proceeding without message edit.`);
        }

        const withdrawalRequest = await createWithdrawalRequest(userId, amountLamports, feeLamports, recipientAddress);
        ...
        try {
            ...
            const balanceUpdateResult = await updateUserBalanceAndLedger(
                deductClient, userId, -totalDeductionLamports,
                'withdrawal_initiated', { withdrawalId }, `Withdrawal ID ${withdrawalId} initiated`
            );
            if (!balanceUpdateResult.success) {
                await deductClient.query('ROLLBACK'); 
                await updateWithdrawalStatus(withdrawalId, 'failed', null, null, `Failed balance deduction: ${balanceUpdateResult.error}`);
            ...
            if (deductClient) await deductClient.query('ROLLBACK').catch(()=>{}); 
            await updateWithdrawalStatus(withdrawalId, 'failed', null, null, `Failed balance deduction in confirm: ${deductError.message}`);
            throw deductError; 
        } finally {
            if (deductClient) deductClient.release();
        }

        await addPayoutJob({ type: 'payout_withdrawal', withdrawalId: withdrawalId, userId });
        ...
        const queuedMessage = `✅ Your withdrawal request for ${escapeMarkdownV2(formatSol(amountLamports))} SOL to \`${escapeMarkdownV2(recipientAddress)}\` has been queued and will be processed shortly\\.\nYou will receive another message upon completion\\.`;
        
        if (confirmationMessageId) {
            await bot.editMessageText(queuedMessage, { chat_id: chatId, message_id: confirmationMessageId, parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard })
               .catch(async e => {
                   console.warn(`${logPrefix} Failed to edit message ${confirmationMessageId} to 'queued', sending new. Error: ${e.message}`);
                   await safeSendMessage(chatId, queuedMessage, {parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard });
               });
        } else {
            await safeSendMessage(chatId, queuedMessage, {parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard });
        }
        ...
        console.error(`${logPrefix} Error during withdrawal confirmation: ${error.message}`, error); 
        const errorMsgText = `❌ Error processing your withdrawal: ${escapeMarkdownV2(error.message)}\\. Please try again or contact support if the issue persists\\.`;
        
        if (confirmationMessageId) {
            await bot.editMessageText(errorMsgText, { chat_id: chatId, message_id: confirmationMessageId, parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard })
               .catch(async e => {
                   console.warn(`${logPrefix} Failed to edit message ${confirmationMessageId} with final error, sending new. Error: ${e.message}`);
                   await safeSendMessage(chatId, errorMsgText, {parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard });
               });
        } else {
            await safeSendMessage(chatId, errorMsgText, {parse_mode: 'MarkdownV2', reply_markup: backToWalletKeyboard });
        }
...
// ... (escapeMarkdownV2, formatSol, getUserWalletDetails, getTotalReferralEarnings, generateReferralCode, safeSendMessage, clearUserState, queryDatabase, pool, bot, constants, GAME_CONFIG etc. are defined elsewhere)
...
                    .catch(async e => { if (!e.message.includes("message is not modified")) await safeSendMessage(chatId, noHistoryMsg, options); }); // safeSendMessage from Part 3
        }
        return await safeSendMessage(chatId, noHistoryMsg, options);
    }
...
    if (isFromCallback && messageToEditId) {
        await bot.editMessageText(historyMsg, {chat_id: chatId, message_id: messageToEditId, ...options})
            .catch(async e => { if (!e.message.includes("message is not modified")) await safeSendMessage(chatId, historyMsg, options); });
    } else {
        await safeSendMessage(chatId, historyMsg, options);
    }
}
...
        const milestonePercent = escapeMarkdownV2((REFERRAL_MILESTONE_REWARD_PERCENT * 100).toFixed(1));

        let tiersDisplay = REFERRAL_INITIAL_BONUS_TIERS.map(t => {
            ...
            return `       ▫️ ${countPrefix} *${tierPercent}\%*`; // Escaped %
        }).join('\n');
...
        if (isFromCallback && messageToEditId) {
             bot.editMessageText(errorText, { chat_id: chatId, message_id: messageToEditId, parse_mode: 'MarkdownV2', reply_markup: fallbackKeyboard }).catch(async ()=>{ 
...
        const confirmationLevelEscaped = escapeMarkdownV2(DEPOSIT_CONFIRMATION_LEVEL);
...
        await safeSendMessage(chatId, "Failed to initiate withdrawal process\\. Please try again\\.", { parse_mode: 'MarkdownV2' }); 
        return;
    }
...
        const keyboard = {inline_keyboard: [[{text: "↩️ Back to Menu", callback_data: "menu:main"}]]}; 
        await bot.editMessageText(errorMsg, {chat_id: chatId, message_id: workingMessageId, parse_mode: 'MarkdownV2', reply_markup: keyboard})
            .catch(async e => await safeSendMessage(chatId, errorMsg, {parse_mode: 'MarkdownV2', reply_markup: keyboard})); 
        clearUserState(userId); 
    }
}
...
         if (isFromCallback && messageId) {
             bot.editMessageText(errorText, { chat_id: chatId, message_id: messageId, parse_mode: 'MarkdownV2', reply_markup: fallbackKeyboard }).catch(async ()=>{ 
                 await safeSendMessage(chatId, errorText, { parse_mode: 'MarkdownV2', reply_markup: fallbackKeyboard });
             });
         } else {
             await safeSendMessage(chatId, errorText, { parse_mode: 'MarkdownV2', reply_markup: fallbackKeyboard });
