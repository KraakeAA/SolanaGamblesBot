// lib/solana-connection.js (Corrected Constructor Order)

import { Connection } from '@solana/web3.js';

class RateLimitedConnection extends Connection {
    /**
     * Creates a rate-limited connection that rotates through multiple RPC endpoints.
     * @param {string[]} endpoints - An array of RPC endpoint URLs.
     * @param {object} options - Configuration options.
     * @param {string} [options.commitment='confirmed'] - Default commitment level.
     * @param {object} [options.httpHeaders={}] - Additional HTTP headers.
     * @param {string} [options.wsEndpoint] - Optional WebSocket endpoint (uses the one corresponding to the current HTTP endpoint if not specified).
     * @param {string} [options.clientId='multi-rpc-connection'] - Client identifier for headers.
     * @param {number} [options.maxConcurrent=5] - Max parallel requests.
     * @param {number} [options.retryBaseDelay=600] - Initial delay for retries (ms).
     * @param {number} [options.maxRetries=5] - Max attempts per request (per endpoint cycle).
     * @param {number} [options.rateLimitCooloff=5000] - Min pause duration after hitting 429 (ms).
     * @param {number} [options.retryMaxDelay=30000] - Max delay for exponential backoff (ms).
     * @param {number} [options.retryJitter=0.2] - Percentage jitter for retry delay (0.2 = +/- 20%).
     */
    constructor(endpoints, options = {}) {
        // --- 1. Validate Inputs ---
        if (!Array.isArray(endpoints) || endpoints.length === 0) {
            throw new Error("RateLimitedConnection requires a non-empty array of endpoint URLs.");
        }

        // --- 2. Prepare Arguments for super() ---
        const initialEndpoint = endpoints[0]; // Use the first endpoint initially
        const clientId = options.clientId || `SolanaGamblesBot/2.1-multi-rpc (${process.env.RAILWAY_ENVIRONMENT ? 'railway' : 'local'})`; // Default client ID

        const rpcOptions = {
            commitment: options.commitment || 'confirmed',
            httpHeaders: {
                'Content-Type': 'application/json',
                'solana-client': clientId, // Set the client header internally
                ...(options.httpHeaders || {})
            },
            wsEndpoint: options.wsEndpoint || undefined, // Allow explicit WS endpoint
            disableRetryOnRateLimit: true // <<< Let OUR class handle retries/rate limits
        };

        // --- 3. Call super() FIRST ---
        console.log(`[RLC] Calling super() with initial endpoint: ${initialEndpoint}`);
        super(initialEndpoint, rpcOptions);
        console.log(`[RLC] super() call finished.`);

        // --- 4. Assign 'this' properties AFTER super() ---
        this.endpoints = [...endpoints]; // Store all endpoints
        this.currentEndpointIndex = 0;   // Start with the first endpoint index

        // --- Configuration ---
        this.options = {
            maxConcurrent: options.maxConcurrent || 5,
            retryBaseDelay: options.retryBaseDelay || 600,
            maxRetries: options.maxRetries || 5,
            rateLimitCooloff: options.rateLimitCooloff || 5000,
            retryMaxDelay: options.retryMaxDelay || 30000,
            retryJitter: options.retryJitter || 0.2,
            // Store the explicitly passed wsEndpoint option if needed later
            wsEndpoint: options.wsEndpoint || undefined,
            // Store the explicitly passed httpHeaders if needed later
            httpHeaders: options.httpHeaders || {}
        };

        // --- State ---
        this.requestQueue = [];      // Holds { method, args, resolve, reject, retries, originalEndpointIndex }
        this.activeRequests = 0;     // Current number of in-flight requests
        this.lastRateLimitTime = 0;  // Timestamp of the last 429 error
        this.consecutiveRateLimits = 0; // Counter for consecutive 429s
        this.cooloffTimeout = null;  // Timeout handle for rate limit cooloff period

        // --- Statistics ---
        this.stats = {
            totalRequestsEnqueued: 0,
            totalRequestsFailed: 0,
            totalRequestsSucceeded: 0,
            rateLimitEvents: 0,
            endpointRotations: 0
        };

        // --- Initialization ---
        this._wrapRpcMethods();
        console.log("[RLC] RateLimitedConnection (Multi-RPC) initialized with options:", this.options);
        console.log(`[RLC] Endpoints configured: ${this.endpoints.join(', ')}`);
    }

    // --- Multi-RPC Methods ---

    /**
     * Gets the currently active RPC endpoint URL.
     * @returns {string} The current endpoint URL.
     */
    getCurrentEndpoint() {
        return this.endpoints[this.currentEndpointIndex];
    }

    /**
     * Gets the number of configured RPC endpoints.
     * @returns {number} The number of endpoints.
     */
    getEndpointCount() {
        return this.endpoints.length;
    }

    /**
     * Rotates to the next available RPC endpoint.
     * Updates the internal endpoint used by the Connection.
     * @returns {string} The new current endpoint URL.
     */
    _rotateEndpoint() {
        if (this.endpoints.length <= 1) {
            // console.warn("[RLC] Rotation requested, but only one endpoint is configured."); // Reduce noise
            return this.getCurrentEndpoint();
        }

        const oldIndex = this.currentEndpointIndex;
        this.currentEndpointIndex = (this.currentEndpointIndex + 1) % this.endpoints.length;
        const newEndpoint = this.endpoints[this.currentEndpointIndex];

        // **Critical Step:** Update the endpoint the parent Connection uses.
        // Directly updating the public property seems to work for subsequent requests
        // based on how _rpcRequest in web3.js uses this.rpcEndpoint.
        const oldRpcEndpoint = this.rpcEndpoint;
        this.rpcEndpoint = newEndpoint;

        // Update WebSocket endpoint if not explicitly set
        // We need to update the internal _wsEndpoint and potentially reconnect if needed.
        // Directly setting _wsEndpoint might not be enough if the ws connection is active.
        // For simplicity now, we just update the property. A full solution might
        // require managing the WebSocket connection lifecycle manually upon rotation.
        if (!this.options.wsEndpoint && this._wsEndpoint) {
            try {
                const url = new URL(newEndpoint);
                url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:';
                const newWsEndpoint = url.toString();
                if (this._wsEndpoint !== newWsEndpoint) {
                     console.log(`[RLC] Updating internal WebSocket endpoint reference to: ${newWsEndpoint}`);
                     this._wsEndpoint = newWsEndpoint;
                     // TODO: Consider closing existing WS connection and reopening if necessary
                     // this.disconnect(); // Or a more targeted WS disconnect/reconnect
                     // this.connect();
                }
            } catch(e) {
                console.error(`[RLC] Failed to derive WebSocket endpoint from ${newEndpoint}`, e);
            }
        }


        this.stats.endpointRotations++;
        console.log(`[RLC] Rotated RPC endpoint from ${oldRpcEndpoint} (Index ${oldIndex}) to ${newEndpoint} (Index ${this.currentEndpointIndex}). Rotations: ${this.stats.endpointRotations}`);

        // Reset rate limit counters when rotating
        this.resetRateLimitCounters();

        return newEndpoint;
    }

    // --- Overridden Methods ---

    _wrapRpcMethods() {
        // List of common RPC methods to wrap (add others if needed)
        const rpcMethods = [
            'getAccountInfo', 'getBalance', 'getBlock', 'getBlockHeight',
            'getBlockProduction', 'getBlockCommitment', 'getBlocks', 'getBlocksWithLimit',
            'getBlockTime', 'getClusterNodes', 'getEpochInfo', 'getEpochSchedule',
            'getFeeForMessage', 'getFirstAvailableBlock', 'getGenesisHash', 'getHealth',
            'getIdentity', 'getInflationGovernor', 'getInflationRate', 'getInflationReward',
            'getLargestAccounts', 'getLatestBlockhash', 'getLeaderSchedule', 'getMaxRetransmitSlot',
            'getMaxShredInsertSlot', 'getMinimumBalanceForRentExemption', 'getMultipleAccounts',
            'getProgramAccounts', 'getRecentPerformanceSamples', 'getSignaturesForAddress',
            'getSignatureStatuses', 'getSlot', 'getSlotLeader', 'getSlotLeaders',
            'getStakeActivation', 'getStakeMinimumDelegation', 'getSupply', 'getTokenAccountBalance',
            'getTokenAccountsByDelegate', 'getTokenAccountsByOwner', 'getTokenLargestAccounts',
            'getTokenSupply', 'getTransaction', 'getTransactionCount', 'getVersion',
            'getVoteAccounts', 'isBlockhashValid', 'requestAirdrop', 'sendTransaction',
             // Added methods often used with sendTransaction
             'confirmTransaction', 'getParsedTransaction', 'getParsedTransactions', 'simulateTransaction'
             // Consider adding methods related to Address Lookup Tables if used
        ];

        // Get all methods from the Connection prototype
        const allProtoMethods = Object.getOwnPropertyNames(Connection.prototype)
            .filter(prop => typeof Connection.prototype[prop] === 'function' && prop !== 'constructor');

        // Combine explicitly listed methods with all prototype methods for broader coverage
        const uniqueMethods = [...new Set([...rpcMethods, ...allProtoMethods])];


        uniqueMethods.forEach(method => {
            // Ensure we don't overwrite essential non-RPC methods or the constructor
             if (typeof super[method] === 'function' && method !== 'constructor') {
                 // Store original method reference if needed, though super[method] should work
                 // const originalMethod = super[method];

                 this[method] = (...args) => {
                     // Don't wrap internal methods starting with '_' unless necessary
                     if (method.startsWith('_') && method !== '_rpcRequest') { // Allow wrapping _rpcRequest if needed, but generally avoid others
                         // console.warn(`[RLC] Calling internal method directly (not rate-limited): ${method}`); // Reduce noise
                         return super[method](...args);
                     }
                     // Always enqueue public methods
                     return this._enqueueRequest(method, args);
                 };
             }
        });
    }

    async _enqueueRequest(method, args) {
        this.stats.totalRequestsEnqueued++;
        return new Promise((resolve, reject) => {
            const request = {
                method,
                args,
                resolve,
                reject,
                retries: 0,
                timestamp: Date.now(), // For potential timeout logic later
                originalEndpointIndex: this.currentEndpointIndex // Track starting endpoint for this request cycle
            };

            // Add request to the queue
            this.requestQueue.push(request);

            // Use setImmediate to avoid blocking the event loop if queue processing is synchronous
            setImmediate(() => this._processQueue());
        });
    }

    async _processQueue() {
        // Check conditions preventing processing
        if (this.activeRequests >= this.options.maxConcurrent) {
            return; // Max concurrency reached
        }
        if (this.requestQueue.length === 0) {
            return; // Queue empty
        }

        // Check rate limit cooloff ONLY if consecutive rate limits > 0
        if (this.consecutiveRateLimits > 0) {
             const timeSinceLastRateLimit = Date.now() - this.lastRateLimitTime;
             const requiredCooloff = this.options.rateLimitCooloff * Math.pow(2, this.consecutiveRateLimits -1); // Exponential cooloff based on consecutive fails
             const cappedCooloff = Math.min(requiredCooloff, this.options.retryMaxDelay); // Cap cooloff

             if (timeSinceLastRateLimit < cappedCooloff) {
                  if (!this.cooloffTimeout) {
                      const remainingCooloff = cappedCooloff - timeSinceLastRateLimit;
                      // console.log(`[RLC] Cooling off for ${remainingCooloff.toFixed(0)}ms more (Consecutive RL: ${this.consecutiveRateLimits})...`); // Reduce noise
                       this.cooloffTimeout = setTimeout(() => {
                           this.cooloffTimeout = null;
                           this._processQueue();
                       }, remainingCooloff + 50); // +50ms buffer
                  }
                  return; // Still cooling off
             } // else {
               // Cooloff finished, don't reset counter here, reset on success/rotation
               // console.log("[RLC] Rate limit cooloff finished."); // Reduce noise
             //}
        }


        this.activeRequests++;
        const request = this.requestQueue.shift();

        // Ensure the connection's internal rpcEndpoint matches our manager's current one
        // This might be needed if rotation happened while the request was queued, although
        // direct assignment in _rotateEndpoint should handle this.
        if (this.getCurrentEndpoint() !== this.rpcEndpoint) {
            console.warn(`[RLC] Mismatch between current endpoint (${this.getCurrentEndpoint()}) and connection's (${this.rpcEndpoint}). Updating connection endpoint before request.`);
            this.rpcEndpoint = this.getCurrentEndpoint();
             // Potentially update WS endpoint reference too if needed, similar to _rotateEndpoint logic
        }

        try {
            // --- Execute the actual RPC call using the parent Connection method ---
             const jitter = Math.random() * 100; // 0-100ms jitter
             await new Promise(r => setTimeout(r, jitter));

            // console.log(`[RLC] Executing: ${request.method} on ${this.rpcEndpoint} (Req #${this.stats.totalRequestsSucceeded + this.stats.totalRequestsFailed + 1})`); // Verbose log
            const result = await super[request.method](...request.args);

            // --- Success ---
            request.resolve(result);
            this.stats.totalRequestsSucceeded++;
            // Reset consecutive rate limit counter on success
            if(this.consecutiveRateLimits > 0) {
                // console.log(`[RLC] Resetting consecutive rate limit counter after success.`); // Reduce noise
                this.consecutiveRateLimits = 0;
            }

        } catch (error) {
            // --- Error Handling & Retry Logic ---
            console.warn(`[RLC] Error in ${request.method} on ${this.rpcEndpoint} (Attempt ${request.retries + 1}):`, error.message);

            const isRpcSpecific = this._isRpcSpecificError(error);
            const shouldRetry = this._shouldRetry(error, request.retries, isRpcSpecific); // Pass RPC specific flag

            if (shouldRetry) {
                 // --- Retry ---
                 request.retries++;

                 // Rotate Endpoint if it's an RPC-specific error, we have alternatives,
                 // and we haven't already cycled through all endpoints for this request attempt
                 let rotated = false;
                 if (isRpcSpecific && this.endpoints.length > 1) {
                     // Avoid rotating back to the original endpoint immediately unless it's the only option left
                     const nextIndex = (this.currentEndpointIndex + 1) % this.endpoints.length;
                     if(nextIndex !== request.originalEndpointIndex || this.endpoints.length === 2) { // Rotate if >2 endpoints or if next isn't original
                        this._rotateEndpoint();
                        rotated = true;
                     } else {
                         console.log(`[RLC] Avoiding immediate rotation back to original endpoint (Index ${request.originalEndpointIndex}) for request ${request.method}. Retrying on current.`);
                     }
                 }

                 const delay = this._calculateRetryDelay(request.retries);
                 console.log(`[RLC] Retrying ${request.method} after ${delay.toFixed(0)}ms delay (Retry ${request.retries}/${this.options.maxRetries}).${rotated ? ` Rotated to: ${this.getCurrentEndpoint()}.` : ''}`);

                 try {
                     await new Promise(resolve => setTimeout(resolve, delay));
                     this._requeueRequest(request); // Requeue at the front
                 } catch (requeueError) {
                     console.error(`[RLC] Error during requeue wait/logic for ${request.method}:`, requeueError);
                     this.stats.totalRequestsFailed++;
                     request.reject(error); // Reject with the original error
                 }

            } else {
                 // --- Do Not Retry: Final Failure ---
                 console.error(`[RLC] Request ${request.method} failed permanently after ${request.retries} retries. Last endpoint: ${this.rpcEndpoint}. Error: ${error.message}`);
                 this.stats.totalRequestsFailed++;
                 request.reject(error); // Reject the promise
            }
        } finally {
            // --- Cleanup ---
            this.activeRequests--;
            // Trigger processing the next item immediately
            setImmediate(() => this._processQueue());
        }
    }

     /**
      * Calculates the delay for the next retry attempt using exponential backoff and jitter.
      * @param {number} retryCount - The number of retries already attempted (1-indexed for calculation).
      * @returns {number} Delay in milliseconds.
      */
     _calculateRetryDelay(retryCount) {
         const baseDelay = this.options.retryBaseDelay;
         const maxDelay = this.options.retryMaxDelay;
         const jitterRatio = this.options.retryJitter;

         // Factor in consecutive rate limits for potentially longer delays
         const backoffFactor = Math.pow(2, this.consecutiveRateLimits); // Exponential based on consecutive fails
         const exponentialDelay = baseDelay * Math.pow(2, retryCount -1) * backoffFactor; // retryCount is 1-based here

         const cappedDelay = Math.min(exponentialDelay, maxDelay);
         const jitterMagnitude = cappedDelay * jitterRatio;
         const jitter = (Math.random() * 2 - 1) * jitterMagnitude; // +/- jitter
         const finalDelay = Math.max(50, cappedDelay + jitter); // Ensure minimum delay

         // If we were rate limited, ensure minimum cooloff period is respected
         // (This check might be redundant if cooloff is handled in _processQueue, but adds safety)
          if (this.consecutiveRateLimits > 0) {
               const requiredCooloff = this.options.rateLimitCooloff * Math.pow(2, this.consecutiveRateLimits -1);
               const cappedCooloff = Math.min(requiredCooloff, maxDelay);
              return Math.max(finalDelay, cappedCooloff);
          }

         return finalDelay;
     }

      /**
       * Determines if a request should be retried based on the error and retry count.
       * @param {Error | any} error - The error received.
       * @param {number} retryCount - The number of retries already attempted (0-indexed).
       * @param {boolean} isRpcSpecific - Whether the error is deemed RPC-specific.
       * @returns {boolean} True if the request should be retried, false otherwise.
       */
      _shouldRetry(error, retryCount, isRpcSpecific) {
          if (retryCount >= this.options.maxRetries) {
              return false; // Max retries exceeded
          }

          const errorMessage = error?.message?.toLowerCase() || '';
          const errorStatus = error?.response?.status || error?.statusCode || error?.code;

          // Check for Rate Limit (429)
          if (errorStatus === 429 || String(errorStatus) === '429' || errorMessage.includes('429') || errorMessage.includes('rate limit') || errorMessage.includes('too many requests')) {
               // Update rate limit stats only if not within cooloff from *last* 429 event to avoid rapid increments
               if (this.lastRateLimitTime === 0 || (Date.now() - this.lastRateLimitTime) >= this.options.rateLimitCooloff) {
                  this.consecutiveRateLimits++;
                  this.stats.rateLimitEvents++;
                  console.warn(`[RLC] Rate limit detected! Consecutive: ${this.consecutiveRateLimits}. Cooling off.`);
               }
               this.lastRateLimitTime = Date.now(); // Always update timestamp
              return true;
          }

          // Check for common retryable network/server errors (Codes or Status)
          const retryableStatuses = [500, 502, 503, 504]; // Server-side issues
          if (retryableStatuses.includes(Number(errorStatus))) {
               // Reset consecutive *rate limits* if it's a different server error
               // this.consecutiveRateLimits = 0; // Let rotation handle reset
              return true;
          }

          // Check common retryable network error messages/codes
           const retryableMessages = ['timeout', 'timed out', 'econnreset', 'esockettimedout', 'network error', 'fetch', 'socket hang up', 'connection terminated', 'econnrefused', 'failed to fetch', 'connection refused'];
           const retryableCodes = ['etimedout', 'econnreset', 'enetunreach', 'eai_again', 'econnaborted', 'econnrefused'];

           if (retryableMessages.some(msg => errorMessage.includes(msg)) || (error?.code && retryableCodes.includes(error.code.toLowerCase()))) {
               // Reset consecutive *rate limits* if it's a different network error
                // this.consecutiveRateLimits = 0; // Let rotation handle reset
               return true;
           }
           // Retry simulation errors only if deemed RPC specific (potential temporary load)
            if (isRpcSpecific && (errorMessage.includes('transaction simulation failed') || errorMessage.includes('failed to simulate'))) {
                console.log(`[RLC] Retrying simulation failure (potential temporary RPC load).`);
                return true;
            }


          // If none of the above, assume not retryable
          return false;
      }

       /**
        * Checks if an error is likely specific to the RPC endpoint (and warrants rotation).
        * @param {Error | any} error - The error received.
        * @returns {boolean} True if the error likely warrants RPC rotation.
        */
       _isRpcSpecificError(error) {
           const errorMessage = error?.message?.toLowerCase() || '';
           const errorStatus = error?.response?.status || error?.statusCode || error?.code;

           // Rate limits are endpoint specific
           if (errorStatus === 429 || String(errorStatus) === '429' || errorMessage.includes('429') || errorMessage.includes('rate limit') || errorMessage.includes('too many requests')) {
               return true;
           }
           // Server errors (5xx) are endpoint specific
           if ([500, 502, 503, 504].includes(Number(errorStatus))) {
                return true;
           }
           // Connection timeouts / refusals / resets are endpoint specific
            const networkErrorMessages = ['timeout', 'timed out', 'econnreset', 'esockettimedout', 'econnrefused', 'failed to fetch', 'socket hang up', 'connection refused'];
            const networkErrorCodes = ['etimedout', 'econnreset', 'econnaborted', 'econnrefused'];
            if (networkErrorMessages.some(msg => errorMessage.includes(msg)) || (error?.code && networkErrorCodes.includes(error.code.toLowerCase())) ) {
                return true;
            }
           // Simulation failures can sometimes be endpoint specific load issues
           if (errorMessage.includes('transaction simulation failed') || errorMessage.includes('failed to simulate')) {
                return true;
           }
           // Health check failures
            if (errorMessage.includes('health check failed')) {
                return true;
            }

           return false; // Assume other errors (like bad request 400, blockhash not found 410) are not endpoint-specific
         }

    /**
     * Re-adds a request to the queue, typically at the front for retries.
     * @param {object} request - The request object to requeue.
     */
    _requeueRequest(request) {
        // Requeue at the front for immediate reprocessing after delay
        this.requestQueue.unshift(request);
    }

    // --- Utility Methods ---

    /**
     * Gets current statistics about the connection and queue.
     * @returns {object} Connection statistics.
     */
    getRequestStats() {
        const completedRequests = this.stats.totalRequestsSucceeded + this.stats.totalRequestsFailed;
        const successRate = completedRequests > 0
            ? ((this.stats.totalRequestsSucceeded) / completedRequests * 100).toFixed(2)
            : 'N/A';

        return {
            options: { // Include key options for context
                maxConcurrent: this.options.maxConcurrent,
                maxRetries: this.options.maxRetries,
                rateLimitCooloff: this.options.rateLimitCooloff,
                retryBaseDelay: this.options.retryBaseDelay,
                endpointsCount: this.endpoints.length
            },
            status: {
                queueSize: this.requestQueue.length,
                activeRequests: this.activeRequests,
                currentEndpoint: this.getCurrentEndpoint(),
                currentEndpointIndex: this.currentEndpointIndex,
                consecutiveRateLimits: this.consecutiveRateLimits,
                lastRateLimit: this.lastRateLimitTime > 0
                    ? `${Math.floor((Date.now() - this.lastRateLimitTime) / 1000)}s ago`
                    : 'never',
                 isCoolingOff: !!this.cooloffTimeout
            },
            stats: {
                 totalEnqueued: this.stats.totalRequestsEnqueued,
                 totalSucceeded: this.stats.totalRequestsSucceeded,
                 totalFailed: this.stats.totalRequestsFailed,
                 rateLimitEvents: this.stats.rateLimitEvents,
                 endpointRotations: this.stats.endpointRotations,
                 successRate: `${successRate}%` // Success rate based on completed requests
            }
        };
    }

    /**
    * Resets rate limit counters externally if needed.
    */
    resetRateLimitCounters() {
        // console.log("[RLC] Resetting rate limit counters."); // Reduce noise
        this.consecutiveRateLimits = 0;
        // Don't reset lastRateLimitTime here, as cooloff calculation might still need it briefly
        // Clear cooloff timeout if it exists
        if (this.cooloffTimeout) {
            clearTimeout(this.cooloffTimeout);
            this.cooloffTimeout = null;
        }
    }
}

export default RateLimitedConnection;
