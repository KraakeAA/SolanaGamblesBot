...
class RateLimitedConnection extends Connection {
    ...
    _wrapRpcMethods() {
        ...
        uniqueMethods.forEach(method => {
         ...
         // Ensure specific methods like `_rpcRequest` are accessible if needed internally
         // (though direct use should be avoided in favor of wrapped methods)
         this._rpcRequest = super._rpcRequest.bind(this);
    }
    ...
    async _processQueue() {
        ...
        if (this.activeRequests >= this.options.maxConcurrent) {
            // console.log(`Queue limit reached (${this.activeRequests}/${this.options.maxConcurrent})`); // Debug log
            return;
        }
        if (this.requestQueue.length === 0) {
             // console.log("Queue empty"); // Debug log
            return;
        }
        const timeSinceLastRateLimit = Date.now() - this.lastRateLimitTime;
        if (timeSinceLastRateLimit < this.options.rateLimitCooloff) {
             // console.log(`Cooling off from rate limit for ${this.options.rateLimitCooloff - timeSinceLastRateLimit}ms`); // Debug log
             // Schedule a check after the cooloff period ends
             setTimeout(() => this._processQueue(), this.options.rateLimitCooloff - timeSinceLastRateLimit + 50); // +50ms buffer
            return;
        }

        this.activeRequests++;
        const request = this.requestQueue.shift();
        // console.log(`Processing: ${request.method}, Active: ${this.activeRequests}, Queue: ${this.requestQueue.length}`); // Debug log

        try {
            // --- Execute the actual RPC call ---
            // Add slight jitter before execution
            ...
            if (this._shouldRetry(error, request.retries)) {
                ...
                // IMPORTANT: Return here after scheduling retry, do not fall through to reject/finally
                // The finally block below will handle decrementing activeRequests correctly.
            ...
            // Use setImmediate to avoid potential deep recursion / stack overflow issues
            setImmediate(() => this._processQueue());
        }
    }
    ...
    _calculateRetryDelay(retryCount) {
        ...
        // Apply jitter: Calculate deviation range based on percentage
        const jitterMagnitude = cappedDelay * this.options.retryJitter;
        const jitter = (Math.random() * 2 - 1) * jitterMagnitude; // Random value between -jitterMagnitude and +jitterMagnitude

        return Math.max(0, cappedDelay + jitter); // Ensure delay is not negative
    }
    ...
    _shouldRetry(error, retryCount) {
        if (retryCount >= this.options.maxRetries) {
             // console.log("Retry limit reached."); // Debug log
            return false;
        }

        const errorMessage = error?.message?.toLowerCase() || '';
        const errorCode = error?.code?.toLowerCase(); // Get error code if available
        const errorStatus = error?.response?.status || error?.statusCode; // Get HTTP status if available
        ...
        if (errorStatus === 429 || errorMessage.includes('429') || errorMessage.includes('rate limit') || errorMessage.includes('too many requests')) {
            ...
            this.stats.rateLimitEvents++;
             console.warn(`Rate limit detected! Consecutive: ${this.consecutiveRateLimits}`);
            return true; // Always retry rate limit errors if within maxRetries
        }
        ...
        const retryableCodes = ['etimedout', 'econnreset', 'enetunreach', 'eai_again', 'econnaborted']; // Added ECONNABORTED

        if (retryableMessages.some(msg => errorMessage.includes(msg)) || (errorCode && retryableCodes.includes(errorCode))) {
            // Reset consecutive rate limit counter if the error is not a rate limit
             this.consecutiveRateLimits = 0;
            return true;
        }

         // Handle potential Solana JSON RPC error structure for specific retryable server errors (e.g., 503)
         // Note: The base web3.js connection might already handle some of these.
        ...
        // If none of the above, don't retry
         // console.log(`Non-retryable error encountered: ${errorMessage} (Code: ${errorCode}, Status: ${errorStatus})`); // Debug log
         this.consecutiveRateLimits = 0; // Reset counter on non-retryable errors too
        return false;
    }
    ...
    _requeueRequest(request) {
        ...
        // More complex logic could adjust position based on request.retries
        this.requestQueue.unshift(request); // Put retrying request at the front
         // console.log(`Requeued ${request.method} (Retry ${request.retries})`); // Debug log
    }
    ...
    getRequestStats() {
        const successRate = this.stats.totalRequestsEnqueued > 0
            ...
            : 'N/A'; // Base success rate on enqueued vs succeeded
    ...
    */
    resetRateLimitCounters() {
        ...
        this.consecutiveRateLimits = 0;
        this.lastRateLimitTime = 0; // Also reset timestamp to allow immediate processing if cooloff was active
    }
}
