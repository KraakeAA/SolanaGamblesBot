...
class RateLimitedConnection extends Connection {
    ...
    async executeWithRetry(method, args, attempt = 1) {
        try {
            return await this._execute(method, args);
        } catch (error) {
            if (this._shouldRetry(error) && attempt <= this.maxRetries) {
    ...
    async _processQueue() {
        ...
        try {
            ...
            if (this._shouldRetry(error) && request.retries < this.maxRetries) {
